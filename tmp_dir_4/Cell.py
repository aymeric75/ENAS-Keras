        ...
        if(self.type_cell=='conv'):
            ...
            if(self.scheme==1):
                ...
                for i in range(0, 2):
                    ...
                    new_block = Block(block_name, input1=int(i), input2=int(i), op1=blocks_array[i], op2=blocks_array[i], inputs=outputs, cell_type=self.type_cell, num_cell=num_cell)
                    new_block.construct()
                    outputs.append(new_block.output)
                for i in range(2, len(blocks_array), step):
                    chosen_outputs.append(blocks_array[i])
                    block_name = 'C'+str(num_cell)+'-B'+str(int((i+2)/step))
                    new_block = Block(block_name, input1=int(i/2), input2=blocks_array[i], op1=blocks_array[i+1], op2=blocks_array[i+1], inputs=outputs[1:], cell_type=self.type_cell, num_cell=num_cell)
                    new_block.construct()
                    outputs.append(new_block.output)
                ...
                for i in range(4, len(blocks_array), step):
                    ...
                    new_block = Block(block_name, input1=int((i+1)/4) + 1, input2=blocks_array[i]+1, op1=blocks_array[i+1], op2=blocks_array[i+2], inputs=outputs, cell_type=self.type_cell, num_cell=num_cell)
                    new_block.construct()
                    outputs.append(new_block.output)
